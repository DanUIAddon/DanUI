...
-- DUI: settings table is created by LFGFilterDB.lua at ADDON_LOADED (DanUIDB may
-- not exist yet at file scope), so the upstream initializer is not needed here.
...
function PGF.MigrateStateV4()
    if DanUICharDB.LFGFilter.version < 4 then
        for k, v in pairs(DanUICharDB.LFGFilter) do
            if type(v) == "table" then
                v.expert = DanUICharDB.LFGFilter.expert
            end
        end
        DanUICharDB.LFGFilter.expert = nil
        DanUICharDB.LFGFilter.version = 4
        print(string.format(L["message.settingsupgraded"], "4"))
    end
end

function PGF.MigrateStateV5()
    if DanUICharDB.LFGFilter.version < 5 then
        local mapping = {
...
function PGF.MigrateStateV6()
    if DanUICharDB.LFGFilter.version < 6 then
        ...
        DanUICharDB.LFGFilter.version = 6
        print(string.format(L["message.settingsupgraded"], "6"))
    end
end

function PGF.MigrateStateV7()
    if DanUICharDB.LFGFilter.version < 7 then
        if DanUICharDB.LFGFilter.c2f4 and DanUICharDB.LFGFilter.c2f4.dungeon then
            DanUICharDB.LFGFilter.c2f4.dungeon.blfit = nil
            DanUICharDB.LFGFilter.c2f4.dungeon.brfit = nil
        end
        DanUICharDB.LFGFilter.version = 7
        print(string.format(L["message.settingsupgraded"], "7"))
    end
end

function PGF.MigrateStateV8()
    if DanUICharDB.LFGFilter.version < 8 then
        -- Add delves support
        if not DanUICharDB.LFGFilter.c121f4 then
            DanUICharDB.LFGFilter.c121f4 = { enabled = true }
        end
        DanUICharDB.LFGFilter.version = 8
        print(string.format(L["message.settingsupgraded"], "8"))
    end
end

function PGF.MigrateSettingsV2()
    if not DanUIDB.LFGFilter.settings.version or DanUIDB.LFGFilter.settings.version < 2 then
        if PGF.IsRetail() then -- disable features now provided by default
            DanUIDB.LFGFilter.settings.classCircle = false
        end
        DanUIDB.LFGFilter.settings.version = 2
    end
end

function PGF.MigrateSettingsV3()
    if not DanUIDB.LFGFilter.settings.version or DanUIDB.LFGFilter.settings.version < 3 then
        DanUIDB.LFGFilter.settings.coloredApplications = nil
        DanUIDB.LFGFilter.settings.version = 3
    end
end
...
function PGF.OnAddonLoaded(name)
    if name == PGFAddonName then
        ...
        PGF.Table_UpdateWithDefaults(DanUIDB.LFGFilter.settings, PGF.C.SETTINGS_DEFAULT)
        PGF.MigrateSettingsV2()
        PGF.MigrateSettingsV3()

        -- initialize dialog state and migrate to latest version
        if DanUICharDB.LFGFilter == nil or DanUICharDB.LFGFilter.version == nil then
            DanUICharDB.LFGFilter = {
                version = 8,
