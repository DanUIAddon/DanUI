...
function PGF.AddRoleIndicators(self, searchResultInfo)
    ...
    if not DanUIDB.LFGFilter.settings.classBar and
       not DanUIDB.LFGFilter.settings.classCircle and
       not DanUIDB.LFGFilter.settings.specIcon and
       not DanUIDB.LFGFilter.settings.leaderCrown and
       not DanUIDB.LFGFilter.settings.missingRoles then
    ...
    for i = 1, #members do
        ...
        frames[i]:Show()
        if DanUIDB.LFGFilter.settings.classBar then
            frames[i].ClassBar:Show()
            frames[i].ClassBar:SetColorTexture(color.r, color.g, color.b, 1)
        end
        if DanUIDB.LFGFilter.settings.classCircle then
        ...
        if DanUIDB.LFGFilter.settings.specIcon then
        ...
        if DanUIDB.LFGFilter.settings.leaderCrown and members[i].isLeader then
    ...
    if DanUIDB.LFGFilter.settings.missingRoles then
