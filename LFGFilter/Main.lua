...
-- How many of the last pass' results PGF could actually read. On 12.1 the client
-- hands a tainted caller a search result table it cannot access at all, so this
-- can be 0 with a full result list on screen - see PGF.FilterSearchResults.
PGF.numResultsReadable = 0
...
-- DUI: per-activity memos.
--
-- The lowercased activity name and the numbers inside it are static game data
-- keyed by activityID, but the result loop rebuilt both - plus a fresh findnumber
-- closure - for every group in the list. A hundred-group dungeon list spans maybe
-- eight distinct activities, so this turns a hundred string allocations, pattern
-- scans and closure builds per refresh into eight for the session.
local activityNameLower = {}
local activityFindNumber = {}

local function GetActivityNameLower(activityID, activityInfo)
    local name = activityNameLower[activityID]
    if not name then
        name = activityInfo.fullName:lower()
        activityNameLower[activityID] = name
    end
    return name
end

local function GetActivityFindNumber(activityID, activityName)
    local finder = activityFindNumber[activityID]
    if not finder then
        local numbers = PGF.String_ExtractNumbers(activityName)
        finder = function (min, max)
        ...
        activityFindNumber[activityID] = finder
    end
    return finder
end
...
function PGF.DoFilterSearchResults(results)
    ...
    for idx = #results, 1, -1 do
        ...
        if searchResultInfo and activityID then
            PGF.numResultsReadable = PGF.numResultsReadable + 1
            ...
            env.activityname = GetActivityNameLower(activityID, activityInfo)
            ...
            env.findnumber = GetActivityFindNumber(activityID, env.activityname)
...
function PGF.ColorGroupTexts(self, searchResultInfo)
    if not DanUIDB.LFGFilter.settings.coloredGroupTexts then return end
    local groupKey = PGF.GetGroupKey(searchResultInfo)
    ...
    if not searchResultInfo.isDelisted then
        ...
        if PGF.IsSoftDeclinedGroup(searchResultInfo) then
            ...
            self.Name:SetTextColor(color.R, color.G, color.B)
            if not DanUIDB.LFGFilter.settings.signUpDeclined then
...
function PGF.FilterSearchResults()
    ...
    -- DanUI patch (2026-09-15): do not publish a results list we could not filter.
    --
    -- On 12.1 the client can hand a *tainted* caller a search result table it cannot
    -- access at all - canaccesstable() says no, so PGF.GetSearchResultInfo returns nil
    -- for every result and DoFilterSearchResults hands back the input copy untouched.
    -- Publishing that copy is then pure cost: it filters nothing, but it writes an
    -- addon-owned table into SearchPanel.results / .totalResults, which taints those
    -- fields. Blizzard's own LFGListSearchPanel onEvent reads them, inherits the taint,
    -- and then dies indexing searchResultInfo.activityIDs (a secret table) inside
    -- LFGListSearchEntry_Update - once per visible entry, per refresh.
    --
    -- Bailing here leaves Blizzard's own untainted list in place, so the group finder
    -- keeps working unfiltered instead of erroring. As soon as even one result reads
    -- back, filtering publishes as before.
    if PGF.numResultsReadable == 0 then
        PGF.Logger:Debug("PGF.FilterSearchResults: no readable results, skipping publish")
        return
    end
