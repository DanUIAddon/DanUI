...
function PGFDialog:OnShow()
    ...
    if not DanUIDB.LFGFilter.settings.dialogMovable then
        self:ResetPosition()
    end
...
function PGFDialog:OnMouseDown(button)
    if not DanUIDB.LFGFilter.settings.dialogMovable then return end
    self:StartMoving()
end
...
function PGFDialog:GetState(id)
    if DanUICharDB.LFGFilter[id] == nil then
        DanUICharDB.LFGFilter[id] = {}
        if self.panels[id] then -- if there is a special panel registered for this id, enable it on first run
            DanUICharDB.LFGFilter[id].enabled = true
        end
    end
    return DanUICharDB.LFGFilter[id]
end
