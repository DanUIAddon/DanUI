...
function DelvePanel:OnLoad()
    ...
    self:HookScript("OnShow", function (self)
        if self.duiDelvesStale then
            self.duiDelvesStale = nil
            self:UpdateDelves()
        end
    end)
...
-- DUI: AREA_POIS_UPDATED fires constantly while out in the world, and each one
-- ran UpdateDelves, which walks every delve zone map through C_AreaPoiInfo and
-- then fuzzy-matches names against fifteen checkboxes. All that work only ever
-- sets the text colour on rows inside this panel, so it is wasted unless the
-- panel is up. The dialog is parented to PVEFrame, so IsVisible() here is
-- precisely "the group finder is open on the delve filters".
