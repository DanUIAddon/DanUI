...
function UsePGFButton:OnLoad()
    ...
    self:SetPoint("LEFT", LFGListFrame.SearchPanel.CategoryName, "RIGHT", -8, 0)
    ...
    self:SetScript("OnEnter", function (self)
        ...
        if PGF.numResultsBeforeFilter > 0 then
            ...
            if PGF.numResultsReadable == 0 then
                -- PGF.FilterSearchResults bailed without publishing, so the list on
                -- screen is Blizzard's own - the removed/displayed counts would read
                -- as "nothing was hidden" when in truth nothing was even looked at.
                GameTooltip:AddLine(L["dialog.usepgf.results.restricted"], nil, nil, nil, true)
            else
