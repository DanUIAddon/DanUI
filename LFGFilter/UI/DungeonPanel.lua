...
function DungeonPanel:OnLoad()
    ...
    -- DUI: flush anything OnEvent deferred while the panel was hidden. The
    -- OnShow method below is invoked by the dialog, not by frame Show(), so
    -- switching category would otherwise leave a stale panel on screen.
    self:HookScript("OnShow", function (self)
        if self.duiFiltersStale and self.state then
            self.duiFiltersStale = nil
...
function DungeonPanel:OnEvent(event)
    ...
    elseif (event == "ACTIVE_PLAYER_SPECIALIZATION_CHANGED" or event == "GROUP_ROSTER_UPDATE") and self.state then
        ...
        -- DUI: GROUP_ROSTER_UPDATE fires on every roster change, so a raid
        -- forming or a dungeon group churning spams it. Neither call below means
        -- anything with the window shut: one swaps two checkboxes in this panel,
        -- the other pushes Blizzard's advanced filter, which only affects a
        -- search started from that window. The dialog is parented to PVEFrame,
        -- so IsVisible() here is precisely "the group finder is open on the
        -- dungeon filters". OnUpdateExpression re-pushes the advanced filter
        -- whenever a filter is touched, so the deferral cannot desync it.
        if self:IsVisible() then
            self:UpdateCheckboxVisibility()
            self:UpdateAdvancedFilters()
        else
            self.duiFiltersStale = true
        end
    end
end
...
function DungeonPanel:OnShow()
    ...
    self.duiFiltersStale = nil
    self:UpdateCheckboxVisibility()
    if self.state then self:UpdateAdvancedFilters() end
end
